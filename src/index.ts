...
class NetworkEverywhereImpl implements NetworkEverywhere {
	...
	async getActiveInterface(): Promise<{ ip: string, mac: string }> {
		...
		if (networkInterfaces[activeIface]) {
			...
			if (iface) {
				return { ip: iface.address, mac: this.formatMacAddress(iface.mac) }
			}
		}
	...
	async getAllMacAddresses(): Promise<string[]> {
		...
		return NetworkEverywhereImpl.pullMac(stdout).map(originalMac => this.formatMacAddress(originalMac))
	}

	private formatMacAddress(originalMac: string): string {
		return originalMac.toLocaleLowerCase().replace(/-/g, ':')
	}
